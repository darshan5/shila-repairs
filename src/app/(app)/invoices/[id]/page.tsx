...
import { ArrowLeft, Plus, Trash2, Printer, Lock, AlertTriangle, Ban } from "lucide-react";
...
export default function InvoiceDetailPage() {
  ...
  async function handleAddItem() {
    ...
    setNewDesc(""); setNewQty("1"); setNewPrice(newCategory === "labor" && hourlyRate > 0 ? hourlyRate.toString() : ""); setNewCategory("labor");
    ...
    <div className="max-w-3xl space-y-6 print:max-w-none print:space-y-4">
      <div className="flex items-center justify-between print:hidden">
        ...
        <div className="flex gap-2">
          ...
          {!isVoid && inv.status !== "finalized" && (
            <button onClick={() => setConfirmFinalize(true)} className="flex items-center gap-1.5 rounded-lg bg-green-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-green-700">
              <Lock className="h-3.5 w-3.5" />Finalize
            </button>
          )}
          {!isVoid && (
            <button onClick={() => setConfirmVoid(true)} className="flex items-center gap-1.5 rounded-lg border border-red-300 px-3 py-1.5 text-sm text-red-600 hover:bg-red-50">
        ...
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 print:hidden">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-amber-800">Finalize this invoice?</p>
              <p className="text-sm text-amber-700 mt-1">This marks the invoice as final. You can still edit it afterwards if needed.</p>
              <div className="flex gap-2 mt-3">
                <button onClick={handleFinalize} className="rounded-lg bg-green-600 px-4 py-1.5 text-sm font-medium text-white hover:bg-green-700">Confirm Finalize</button>
                <button onClick={() => setConfirmFinalize(false)} className="rounded-lg border border-slate-300 px-4 py-1.5 text-sm hover:bg-slate-50">Cancel</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {confirmVoid && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-4 print:hidden">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-red-800">Void this invoice?</p>
              <p className="text-sm text-red-700 mt-1">The invoice will be kept on record but will not count toward totals. This cannot be undone.</p>
              <div className="flex gap-2 mt-3">
                <button onClick={handleVoid} className="rounded-lg bg-red-600 px-4 py-1.5 text-sm font-medium text-white hover:bg-red-700">Confirm Void</button>
                <button onClick={() => setConfirmVoid(false)} className="rounded-lg border border-slate-300 px-4 py-1.5 text-sm hover:bg-slate-50">Cancel</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-lg border">
        <table className="w-full text-sm">
          <thead className="border-b bg-slate-50">
            <tr>
              ...
              {canEdit && <th className="px-3 py-2 w-10 print:hidden" />}
            </tr>
          </thead>
          <tbody>
        ...
        {/* Add item — always available unless voided */}
        {canEdit && (
          <div className="flex gap-2 border-t p-3 print:hidden flex-wrap">
            <input value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="Description" className="flex-1 min-w-[120px] rounded border px-2 py-1 text-sm" />
      ...
      <div className="flex justify-end">
        <div className="w-64 space-y-1 text-sm">
          ...
          <div className={`flex justify-between border-t pt-1 font-bold text-base ${isVoid ? "line-through text-red-400" : ""}`}>
            <span>Total</span><span>${inv.total.toFixed(2)}</span>
          </div>
          {isVoid && <p className="text-xs text-red-500 text-right">Invoice voided — not counted in totals</p>}
        </div>
      </div>
